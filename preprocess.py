...
def build_save_text_dataset_in_shards(src_corpus, src_hist_corpus, src_corpus2, tgt_corpus2, fields,
    ...
    while not src_iter.hit_end():
        ...
        dataset = onmt.io.TextDataset(
                fields, src_iter, src_hist_iter, src_iter2, tgt_iter2,
                src_iter.num_feats, src_hist_iter.num_feats, src_iter2.num_feats, tgt_iter2.num_feats,
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        src_hist_corpus = opt.train_src_hist
        src_corpus2 = opt.train_src
        tgt_corpus2 = opt.train_tgt
        pointers = opt.train_ptr
    else:
        src_corpus = opt.valid_src
        src_hist_corpus = opt.valid_src_hist
        src_corpus2 = opt.valid_src
        tgt_corpus2 = opt.valid_tgt
        pointers = None

    # Currently we only do preprocess sharding for corpus: data_type=='text'.
    if opt.data_type == 'text':
        return build_save_text_dataset_in_shards(
                src_corpus, src_hist_corpus, src_corpus2, tgt_corpus2, fields,
...
def main():
    ...
    src_nfeats1 = onmt.io.get_num_features(opt.data_type, opt.train_src, 'src1')
    src_hist_nfeats1 = onmt.io.get_num_features(opt.data_type, opt.train_src_hist, 'src1_hist')

    src_nfeats2 = onmt.io.get_num_features(opt.data_type, opt.train_src, 'src2')
    tgt_nfeats2 = onmt.io.get_num_features(opt.data_type, opt.train_tgt, 'tgt2')
    print(" * number of source features: %d." % src_nfeats1)
    print(" * number of source history features: %d." % src_hist_nfeats1)
    # print(" * number of source features- stage 2: %d." % src_nfeats2)
    print(" * number of target features: %d." % tgt_nfeats2)

    print("Building `Fields` object...")
    fields = onmt.io.get_fields(opt.data_type, src_nfeats2, tgt_nfeats2, src_hist_nfeats1)
